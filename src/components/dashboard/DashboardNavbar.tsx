...
const DashboardNavbar = ({ openSideBar }: { openSideBar: () => void }) => {
  ...
  return (
    <div className="lg:px-9 px-5 h-[98px] mb-4 lg:mb-0 text-white lg:text-black flex items-center bg-[#884c20] lg:bg-transparent justify-between lg:relative w-full z-50">
      <div className="lg:hidden" onClick={openSideBar}>
        <IoMenu size={25} />
      </div>
      <div className="lg:hidden w-[173px] h-[26px] overflow-hidden">
        <img className="w-full h-full object-contain" src={logo} alt="Logo" />
      </div>
      <div className="flex items-center gap-2 lg:justify-between lg:w-full relative">
        <div>
          <p className="text-2xl font-bold hidden lg:block">Dashboard</p>
        </div>
        <div className="flex relative items-center gap-4">
          <button className="hidden outline-none rounded-full border bg-[#884c20] text-white lg:flex items-center px-3 py-1 gap-1">
            <GoUpload size={20} />
            <Link to={"/uploads"}>
          ...
          <button
            onClick={(e) => { e.stopPropagation(); setProfile(!profile) }}
            className="outline-none rounded-full border border-white lg:border-black p-1"
          >
            <CiUser size={20} />
          </button>
          {profile && (
